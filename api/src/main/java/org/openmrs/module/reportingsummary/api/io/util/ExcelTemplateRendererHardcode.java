 ...
 */
public class ExcelTemplateRendererHardcode {

	protected static final Log log = LogFactory.getLog(ExcelTemplateRendererHardcode.class);
	public static void RenderToExcel(OutputStream os, String type) throws Exception {
		
		// We first set up a report with 2 indicators, numbered 1 and 2
		System.out.println("HARDCODE STARt ----------------------------------------");
	...
	//	props.put("repeatingSections", "sheet:1,dataset:allPatients | sheet:2,column:4,dataset:allPatients | sheet:3,dataset:allPatients");
		props.put("repeatingSections", "sheet:1,row:6-8,dataset:allPatients | sheet:2,column:4,dataset:allPatients | sheet:3,dataset:allPatients");
		design.setProperties(props);
		
		ReportDesignResource resource = new ReportDesignResource();
		resource.setName("template.xls");
		InputStream is = OpenmrsClassLoader.getInstance().getResourceAsStream("org/openmrs/module/reportingsummary/web/controller/ExcelTemplateRendererTest.xls");
		...
		XmlReportRenderer xrr= new XmlReportRenderer();
		...
		log.info("date should be here ----------------------------------------");
		System.out.println("date should be here ----------------------------------------");
		csvRenderer.render(data, "thedata", System.out);
		System.out.println("date should be above ----------------------------------------");
		log.info("date should be above ----------------------------------------");
		//TestUtil.printOutTableContents(getConnection(), "patient");
		//TestUtil.printOutTableContents(getConnection(), "person");
		FileOutputStream xos = new FileOutputStream("/temp/test.xml"); // You will need to change this if you have no /tmp directory
		FileOutputStream cos = new FileOutputStream("/temp/test.csv");
		//renderer.render(data, "xxx:xls", fos);
		if(type.equals("csv")){
			csvRenderer.render(data, "thedata", os);
		}
		else if(type.equals("xml")){
			xrr.render(data, "xxx:xls", os);
		}
		xrr.render(data, "xxx:xls", xos);
		csvRenderer.render(data, "thedata", cos);
//		xrr.render(data, "xxx:xls", os);
		xos.close();
		os.close();
		cos.close();
